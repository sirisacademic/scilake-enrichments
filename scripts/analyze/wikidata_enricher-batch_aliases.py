...
def get_wikidata_aliases_sparql(qid: str, language: str = "en") -> Dict[str, any]:
    ...
    """
    result = get_wikidata_aliases_batch([qid], language)
    return result.get(qid, {'label': None, 'aliases': []})


def get_wikidata_aliases_batch(qids: List[str], language: str = "en") -> Dict[str, Dict[str, any]]:
    """
    Retrieve labels and aliases for multiple Wikidata entities in one SPARQL query.
    ...
    Returns:
        Dictionary mapping QID to {'label': str, 'aliases': List[str]}
    """
    if not qids:
        return {}
    ...
    SELECT ?entity ?label ?alias WHERE {{
      ...
      OPTIONAL {{ ?entity rdfs:label ?label . FILTER(LANG(?label) = "{language}") }}
      OPTIONAL {{ ?entity skos:altLabel ?alias . FILTER(LANG(?alias) = "{language}") }}
    }}
    """
    
    data = sparql_query(query)
    if not data:
        return {qid: {'label': None, 'aliases': []} for qid in qids}
    
    results = data.get('results', {}).get('bindings', [])
    if not results:
        return {qid: {'label': None, 'aliases': []} for qid in qids}
    
    # Parse results by entity
    entity_data = {}
    for row in results:
        entity_uri = row.get('entity', {}).get('value', '')
        qid = entity_uri.split('/')[-1]  # Extract Q123 from URI
        
        if qid not in entity_data:
            entity_data[qid] = {'label': None, 'aliases': set()}
        
        # Get label
        if 'label' in row and entity_data[qid]['label'] is None:
            entity_data[qid]['label'] = row['label']['value']
        
        # Get alias
        if 'alias' in row:
            entity_data[qid]['aliases'].add(row['alias']['value'])
    
    # Convert sets to lists and fill missing QIDs
    result = {}
    for qid in qids:
        if qid in entity_data:
            result[qid] = {
                'label': entity_data[qid]['label'],
                'aliases': list(entity_data[qid]['aliases'])
            }
        else:
            result[qid] = {'label': None, 'aliases': []}
...
class WikidataEnricher:
    ...
    def enrich_taxonomy(self, input_path: str, output_path: str, 
                       resume: bool = True, batch_size: int = 20) -> pd.DataFrame:
        """
        Enrich entire taxonomy file with Wikidata information using batched SPARQL queries.
        
        Args:
            input_path: Path to input TSV file
            output_path: Path to output TSV file
            resume: Whether to resume from checkpoint
            batch_size: Number of QIDs to fetch aliases for in one SPARQL query
        ...
        print(f"   Batch size: {batch_size} QIDs per SPARQL query")
        
        processed = 0
        idx = start_idx
        
        with tqdm(total=len(df) - start_idx, desc="Enriching", initial=0) as pbar:
            while idx < len(df):
                # Collect batch of entries that need processing
                batch_entries = []
                batch_qids = []
                
                for i in range(idx, min(idx + batch_size, len(df))):
                    # Skip if already has both QID and aliases
                    if df.at[i, 'wikidata_id'] and df.at[i, 'wikidata_aliases']:
                        pbar.update(1)
                        continue
                    
                    concept = df.at[i, 'concept']
                    description = df.at[i, 'description']
                    existing_qid = extract_qid(df.at[i, 'wikidata_id'])
                    
                    # Find QID if missing
                    qid = existing_qid
                    if not qid:
                        print(f"  🔍 Finding QID for '{concept}'...", end='', flush=True)
                        qid = self.find_wikidata_id(concept, description)
                        if qid:
                            df.at[i, 'wikidata_id'] = f"https://www.wikidata.org/wiki/{qid}"
                            print(f" ✓ {qid}")
                        else:
                            print(" ✗ Not found")
                            pbar.update(1)
                    ...
                    batch_entries.append((i, concept))
                    batch_qids.append(qid)
                
                # Fetch aliases for batch
                if batch_qids:
                    print(f"  📝 Fetching aliases for {len(batch_qids)} entities...", end='', flush=True)
                    alias_results = get_wikidata_aliases_batch(batch_qids)
                    print(" ✓")
                    
                    # Process results
                    for (row_idx, concept), qid in zip(batch_entries, batch_qids):
                        result = alias_results.get(qid, {'label': None, 'aliases': []})
                        
                        valid_aliases = []
                        
                        # Add label if different from concept
                        if result['label'] and result['label'].lower() != concept.lower():
                            if result['label'].lower() not in existing_terms:
                                valid_aliases.append(result['label'])
                        ...
                        if valid_aliases:
                            valid_aliases = list(dict.fromkeys(valid_aliases))
                            df.at[row_idx, 'wikidata_aliases'] = ' | '.join(valid_aliases)
                            print(f"    {concept}: {len(valid_aliases)} aliases")
                ...
                # Delay between batches (not between individual entries)
                if idx < len(df) and batch_qids:
                    time.sleep(self.delay)
                
                # Save checkpoint every 50 entries
                if processed > 0 and processed % 50 == 0:
                    with open(checkpoint_path, 'wb') as f:
                        pickle.dump({'last_index': idx, 'df': df}, f)
...
Examples:
  ...
  python wikidata_enricher.py --input taxonomy.tsv --output enriched.tsv --threshold 0.7
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help='Number of QIDs to batch per SPARQL query (default: 20)'
    )
    parser.add_argument(
        '--no-resume',
        dest='resume',
    ...
    # Run enrichment
    enricher.enrich_taxonomy(args.input, args.output, resume=args.resume, batch_size=args.batch_size)
